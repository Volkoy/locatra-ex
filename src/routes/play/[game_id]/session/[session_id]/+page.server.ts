...
function selectCard(
	...
	const catMatch = (c: CardRow) =>
		c.character_category === characterCategory || c.character_category === 'both';
	...
	const pools = [
		...
		available.filter((c) => c.card_category === 'general' && stepMatch(c) && catMatch(c)),
		available.filter((c) => c.card_category === 'poi_specific' && c.poi_id === poiId),
...
export const load: PageServerLoad = async ({ params, locals }) => {
	...
	if (locationResult && typeof locationResult === 'object') {
		gameLocation = locationResult as { longitude: number; latitude: number };
	}
	...
	const visitedPoiIds = [...new Set(allSegments.filter((s) => s.poi_id).map((s) => s.poi_id))];
	const hasIntroduction = allSegments.some((s) => s.segment_type === 'introduction');
...
export const actions: Actions = {
	...
	const session = await locals.getSession();
		...
		if (existingSegment) return fail(400, { message: 'POI already visited' });
	...
	const session = await locals.getSession();
		...
		const card_id = form.get('card_id') ? Number(form.get('card_id')) : null;
		const poi_id = Number(form.get('poi_id'));
		...
		const { error: insertError } = await supabase.from('story_segments').insert({
			...
			poi_id,
			card_id,
			hero_step: heroStep as Database['public']['Enums']['hero_steps'] | null,
			segment_type: segment_type as Database['public']['Enums']['segment_type'],
			content,
			segment_order: segmentOrder
		});

		if (insertError) return fail(500, { message: 'Failed to save story segment' });

		await supabase
			.from('game_sessions')
			.update({ current_hero_step: nextHeroStep as Database['public']['Enums']['hero_steps'] | null })
			.eq('id', gameSession.id);

		return { success: true, completed: nextHeroStep === null, nextStep: nextHeroStep ?? null };
	},
